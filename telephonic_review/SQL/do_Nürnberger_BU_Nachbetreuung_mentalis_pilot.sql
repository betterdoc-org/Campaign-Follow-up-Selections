...
-- !! Mentalis Pilot eingeschlossen
-- Die Fälle sollten direkt telefoniert werden, ein result_date ist nicht notwendig.
-- Weil hier ein externes Angebot unabhängig von unserem Service angeboten wird.
-- Der Telefonieprozess sollte nach Labelerstellung direkt umsetzbar sein.
...
trello_mentalis as (
select * from analytics.base_trello_events 
where board = 'Proaktive Steuerung' and label_name = 'Mentalis Pilot'
),
...
final as (
	...
	where 
		...
		and cube_services.case_id not in (
							...
							where 
								case_id is not null
								and cam_batch.cam_id in (109, 57) -- selections nürnberger (mentalis & not mentalis)
							)
...
from
	final
where case_id in (select case_id from trello_mentalis)
)
