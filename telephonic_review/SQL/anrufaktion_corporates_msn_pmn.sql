...
where
	...
	(1 = 1)
	--hard filters
	and (product_v2 ilike '%msn%' or product_v2 ilike '%pmn%')
	...
	and case_id not in (select case_id from md_campaigns where case_id is not null)
	...
	--variable filters
	---------------------------------
	and payer_name ilike '%Osborne Clarke%'
...
select
	...
	'Osborne Clarke GmbH & Co. KG' as cam_name, -----------------------------------------ACHTUNG hier Namen anpassen!!!
	'extra call campaign for our follow ups - Osborne Clarke GmbH & Co. KG' as cam_description, ------------ACHTUNG hier Namen anpassen!!!
